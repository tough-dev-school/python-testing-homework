...
If you are using ``docker`` then prepend ``docker composerun --rm web``
...
.. code:: bash
  ...
  docker composebuild
  docker composerun --rm web python manage.py migrate
  docker composeup
...
we use ``docker composerun`` to run scripts inside docker.
